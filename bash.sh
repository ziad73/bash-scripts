...
# first step to make script executable using chmod(change mode)
#chmod +x temp.sh  


#!/usr/bin/env bash     # '!' called bang
# First line of the script is the shebang which tells the system how to execute, enforce user to use bash shell
# that option offers mobilty between different OS with no issues
...
# Execute a script:
#   ./ vs sh vs source:
#       ./script.sh                     → runs the script in a new shell (needs execute permission).
#       sh script.sh                    → runs the script using sh shell (ignores execute permission, may differ if script uses bash features).
#       source script.sh or . script.sh → runs the script in the current shell, so variables/functions persist after execution.
   
#       Use ./ when script is executable and has #!/bin/bash (or similar).
#       Use sh for portability or quick runs without execution rights.
#       Use source to apply environment changes in your current session.
...
# $* vs $@
# $* store all args in a single string
# $@ store all args in a plain array
...
# Use double brackets [[ ... ]] only once.
if [[ ("$name" == "zuzz" || "$name" == "$USER") && "$age" -gt 19 ]]; then
    echo "name is zuzz/USER and age>19."
fi
...
# -eq -ne -gt -lt -ge -le for numbers comparison
...
# standard streams(bytes transmitted between terminal -> Shell):
#	-Standard Input(stdin) 	0
#	-Standard Output(stdout) 1	
#	-Standard Error(stderr) 2	

# > is '1>' by default, so each time you want to redirect an error use explicit '2>'
# '&>' pass both error and output

# $? know exit code of the previous command
# exit status code:
#	0   → success.
...
# Read from stdin until ^EOF$ and overwrite hello.py with the lines
    # cat > hello.py → overwrite a file named hello.py and writes to it.
    # << EOF         → means read input until the line EOF is found, and put that content into hello.py.
